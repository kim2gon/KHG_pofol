...
const icons = [
  {
    ...
    position: "top-1/4 right-1 lg:top-1/2 lg:right-1/4",
    svg: (
      <svg
  ...
  {
    name: "010-7697-5713",
    position: "top-1/4 right-1/2 lg:top-3/4 lg:right-1/2",
    svg: (
      <svg
        xmlns="http://www.w3.org/2000/svg"
        fill="none"
  ...
  },
  {
    name: "RESUME",
    bg: "black",
    position: "bottom-1/4 left-1 lg:bottom-1/2 lg:left-1/4",
    svg: (
      <svg
        xmlns="http://www.w3.org/2000/svg"
        fill="none"
  ...
  },
  {
    ...
    position: "bottom-1/4 left-1/2 lg:bottom-3/4 lg:left-1/2",
    svg: (
      <svg
        xmlns="http://www.w3.org/2000/svg"
        fill="none"
...
const Section8 = ({ onSectionWheel }) => {
  ...
  try {
    const fileUrl = `${process.env.PUBLIC_URL}/khg_resume.docx`; 
    const response = await fetch(fileUrl);
    const blob = await response.blob();
    const url = window.URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = "김희곤_이력서.docx";
    a.click();
  ...
  const handleIconClick = (name, url) => {
    ...
    if (name === "RESUME") {
      const confirmed = window.confirm("이력서를 다운로드 하시겠습니까?");
      if (confirmed) {
        downloadResume();
      }
      return;
  ...
  const getDivStyle2 = () => ({
    transform:
      divPosition2 === "bottom" ? "translateY(0)" : "translateY(100vh)",
  ...
  return (
    ...
    >
      ...
      <div className="relative w-full h-full flex justify-center items-center">
        <p className="absolute right-[100px] top-20 lg:top-[180px] text-[#111] text-sm leading-[18px]">
      ...
      {isCurrentRoute && (
        <div
          style={getDivStyle2()}
          className="fixed w-screen left-0 bottom-0 pt-32 lg:pt-24 px-[--footerpd] pb-20 flex flex-col items-center text-center bg-black text-white z-50"
        >
