...
const icons = [
  {
    ...
    svg: (
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 98 98" className="size-10 opacity-55">
        <path
    ...
    >
      <div className="relative w-full h-full flex justify-center items-center">
        <p className="m-0 max-w-[200px] absolute right-[100px] top-[180px] font-medium text-[11px] leading-[18px] tracking-wide text-[#111]">
          This is my portfolio and this page introduce my skills. Thank you for visiting!
        </p>
        ...
        >
          ...
          {icons.map(({ name, url, position, svg }) => (
            <div
              key={name}
              onClick={() => handleIconClick(name, url)}
              className={`bigdotspin3 absolute size-[120px] rounded-full ${position} cursor-pointer flex items-center justify-center z-50`}
            >
              {svg}
            </div>
          ))}
